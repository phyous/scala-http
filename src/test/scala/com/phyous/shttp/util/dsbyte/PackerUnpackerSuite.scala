...
class PackerUnpackerSuite extends FunSuite {

    test("testLiteralDecimalIntNormal") {
       val bytes: Array[Byte] = getBytes("<<'123>>")
       assert(bytes(0) === 0x00)
       assert(bytes(1) === 0x00)
       assert(bytes(2) === 0x00)
       assert(bytes(3) === 123)
    ...
    test("testLiteralDecimalIntAbridged") {
    ...
    test("testLiteralStringAbridged")  {
    ...
    test("testPassedIntAndStringFromMap") {
    ...
    test("testIntConvertedToLongOnWrite") {
       val bytes = getBytesFromArgs("<<$0/long>>",1024)
       assert(bytes.length === 8)
       assert(bytes(6) === 0x04)
    }

    test("testWritingBean") {
